...
var SQLiteAdapter = /*#__PURE__*/function () {
  function SQLiteAdapter(options) {
    ...
    this._dispatcher = (0, _makeDispatcher.makeDispatcher)(this._dispatcherType, this._tag, this._dbName, usesExclusiveLocking);
  ...
  _proto._init = function _init(callback) {
    ...
    this._dispatcher.call('initialize', [this._dbName, this.schema.version], function (result) {
      if (result.error) {
        callback(result);
        return;
      }
  ...
  _proto._setUpWithMigrations = function _setUpWithMigrations(databaseVersion, callback) {
    ...
    if (migrationSteps) {
      ...
      this._dispatcher.call('setUpWithMigrations', [this._dbName, require('./encodeSchema').encodeMigrationSteps(migrationSteps), databaseVersion, this.schema.version], function (result) {
        if (result.error) {
  ...
  _proto._setUpWithSchema = function _setUpWithSchema(callback) {
    ...
    this._dispatcher.call('setUpWithSchema', [this._dbName, this._encodedSchema(), this.schema.version], function (result) {
      if (!result.error) {
  ...
  _proto.unsafeExecute = function unsafeExecute(operations, callback) {
    if ('production' !== process.env.NODE_ENV) {
      (0, _common.invariant)(operations && 'object' === typeof operations && 1 === Object.keys(operations).length && Array.isArray(operations.sqls), 'unsafeExecute expects an { sqls: [ [sql, [args..]], ... ] } object');
    }
